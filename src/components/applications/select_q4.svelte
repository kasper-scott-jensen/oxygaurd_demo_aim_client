<script>
    ...
    import {
        ...
        answerResume,
        awardedPointsQ4,
    } from '../../stores/applications' // Import stores for the active answers of questions 2 and 4
    ...
    let answers = [
        {
            award: [1, 2, 4],
            desc: 'You do not currently have, or do not want a compressed air supply.',
        },
        {
            award: [1, 2, 3, 4, 5],
            desc: 'You have a compressed air supply available capable of delivering up to 7 bars of pressure.',
        },
        {
            award: [1, 2, 3, 4, 5],
            desc: 'You have a compressed air supply available capable of delivering up to 3 bars of pressure.',
        },
    ]
...
<div class="p-16 w-full">
    ...
    <p class="mb-6 font-light text-gray-500 md:text-lg max-w-screen-lg">
        The availability and specifications of your air supply are important
        factors in choosing the right dispensing equipment.
    </p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-4">
        {#each answers as answer, index}
            ...
            <div
                on:click|preventDefault={() => {
                    activeAnswerQ4.set(index)
                    handleAwardPoints(index)
                }}
                class={`group flex flex-col w-full justify-start cursor-pointer p-5 bg-white border-2 rounded-lg hover:bg-gray-50 shadow-sm h-40 sm:h-32 ${$activeAnswerQ4 === index ? 'border-primary-700' : 'border-gray-300'}`}
            >
                <p
                    class={`text-gray-900 group-hover:text-primary-700 text-xs uppercase font-semibold mb-2 ${$activeAnswerQ4 === index ? 'text-primary-700' : 'text-gray-500'}`}
                >
                    Answer {index + 1}
                </p>
